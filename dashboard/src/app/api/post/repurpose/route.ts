...
export async function POST(request: NextRequest) {
  try {
    ...
    const { platform, sourceText, sourcePlatform } = body;
    ...
    if (!sourceText || typeof sourceText !== 'string' || sourceText.trim().length === 0) {
      throw AppError.validationError('sourceText is required and cannot be empty');
    }

    if (sourceText.length > 5000) {
      throw AppError.validationError('sourceText must be under 5000 characters');
    }

    const validSourcePlatforms = ['X', 'LinkedIn', 'auto'];
    const resolvedSourcePlatform = sourcePlatform || 'auto';
    if (!validSourcePlatforms.includes(resolvedSourcePlatform)) {
      throw AppError.validationError('sourcePlatform must be X, LinkedIn, or auto');
    }
    ...
    const { systemPrompt, userPrompt } = postService.prepareRepurpose({
      platform: platform as Platform,
      sourceText: sourceText.trim(),
      sourcePlatform: resolvedSourcePlatform as 'X' | 'LinkedIn' | 'auto'
    });

    console.log('🤖 Repurposing post content...');
